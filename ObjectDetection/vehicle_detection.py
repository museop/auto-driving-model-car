...
class VehicleDetector(object):
    def __init__(self, cascade_classifier='Vehicle_HAAR.xml'):
        self.vehicle_cascade = cv2.CascadeClassifier(cascade_classifier)
        self.queue = []
    ...
    def detect(self, image):
        """
        Detect vehicles from the image(BGR image).
        """
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        vehicles = self.vehicle_cascade.detectMultiScale(gray, minNeighbors=5, minSize=(30, 30))

        for (x_pos, y_pos, width, height) in vehicles:
            cv2.putText(image, 'Detected Vehicle', (x_pos-5, y_pos-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
